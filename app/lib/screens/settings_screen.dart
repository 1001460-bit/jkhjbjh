  ...
  Future<void> _load() async {
    try {
      ...
      final futures = <Future>[
        ...
        BackgroundKeepAliveService.instance.init().then((_) => BackgroundKeepAliveService.instance.enabled),
      ...
      final backgroundKeepAlive = results[4] as bool;
      final customSaveDirOrUri = results[5] as String?;
      final receiveResolution = results[6] as ReceiveDirResolution;
      final receiveFallback = results[7] as ReceiveDirFallbackInfo?;
      final windowsLaunchAtStartup = results[8] as bool;
      final profile = results[9] as UserProfile?;
      final membership = results[10] as MembershipMe?;
      logSettings.info(
      ...
      if (mounted) {
        setState(() {
          ...
          _backgroundKeepAlive = backgroundKeepAlive;
                          ...
                          children: [
                            ...
                            if (Platform.isIOS) ...[
                              SwitchListTile(
                                ...
                                onChanged: (v) async {
                                  await BackgroundKeepAliveService.instance
                                      .setEnabled(v);
                                  if (mounted) {
                                    setState(() => _backgroundKeepAlive = v);
                                  }
                                  if (v && mounted) {
                                    await _showSettingsFeatureHintDialog(
                                      title: '后台持续传输',
                                      content:
                                          '开启后，App 退到后台时将申请后台运行时间并维持网络连接，'
                                          '文件传输可以在后台继续进行。关闭时保持原有行为（退后台后系统挂起 App）。',
                                    );
                                  }
                                },
                                title: Text(
